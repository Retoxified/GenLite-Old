...
export class GenLiteNPCHighlightPlugin implements GenLitePlugin {
    ...
    update(dt) {
        ...
        for (let key in npcsToAdd) {
            ...
            text += `
            <div class="genlite-npc-setting" style="display: ${this.isAltDown ? "inline-block" : "none"}; pointer-events: auto;" onclick="document.${GenLiteNPCHighlightPlugin.pluginName}.hide_item('${hpKey}');void(0);"> &#8863;</div>`;
